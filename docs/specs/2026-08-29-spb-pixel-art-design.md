...
### 6.3 风格定稿：写实像素风（2026-08-29，BirdIndex 二轮投产）

Q 版风格上线后用户反馈「像素太低、看不出是什么鸟」，全量改用**写实
像素插画**配方，要点与 6.1 相反——低重绘保形，物种特征靠源图与提示词：

- **denoise 0.35**（0.45 起形态漂移、场景外溢与伪文字风险显著上升；
  0.35 输出≈「源图的白底像素画」，翠鸟蓝橙/杓鹬长嘴等种特征全部保真）。
- **--pixel-grid 128**（模板节点 9/11 同步改 128×128；64 格对写实风格
  过粗，128 格羽纹可辨）。
- 提示词换成写实系：正向 `realistic pixel art of a wild bird / true-to-
  life proportions / accurate natural colors / detailed pixel plumage`，
  负向追加 `chibi, cute, big eyes, cartoon proportions`；批量 item 的
  `prompt` 仍注入英文种名。
- 白底两级 rembg 处理与 6.2 修复继续沿用。

**重要经验：写实风格忠实还原源图——源图差则成品差**（Q 版会"美化"，
写实不会）。462 种按 16×3 拼图逐格目检后约 96 种不合格，其中 40 种换
代表照（按 rating 降序 + 亮度/尺寸/bbox 占比启发式挑候选）重跑后合格；
其余受限于图库现状（照片里有人/拍到树叶/仅头部特写/多只合影/远距过小），
保留原结果，待有更好照片后用同一配方重跑即可。
