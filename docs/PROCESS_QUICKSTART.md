...
**无鸟照片产物瘦身（2026-08-27 起）**：无鸟照片（has_bird=0）**不保留 RAW 预览 JPG、不导出 sidecar JSON**（面向大量无鸟生活照的批次省磁盘）；DB 行仍保留（统计与浏览器「无鸟」筛选依赖它）。副作用：浏览器里无鸟 RAW 没有缩略图。识鸟门控同步放宽为「有鸟即识」——锐度≥100 低地板挡纯糊片、多鸟豁免，稀有鸟软片与补救扫描救回的照片也能进物种召回（旧门控是 置信≥0.5+关键点+锐度≥250 粗筛）。
